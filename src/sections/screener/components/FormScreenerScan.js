...
import { Grid, Stack, TextField, Typography, Box, Checkbox, Select, MenuItem, Button } from '@mui/material';
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setErrors, setSubmitting }) => {
      try {
        ...
        // const keywords = [];
        // if (!_.isEmpty(filterPanelList)) {
        //   _.forEach(filterPanelList, (filterPanel) => {
        //     const dataFill = filterPanel?.data;
        //     dataFill
        //       ?.filter((fill) => fill?.type === 'data')
        //       ?.forEach((data) => {
        //         if (!_.isEmpty(data?.indice)) {
        //           keywords.push(data.indice?.value || data.indice?.name);
        //         }
        //       });
        //   });
        // }
      ...
      <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
        ...
        >
          <Grid item xs={12} lg={12}>
            ...
            >
              <span>Fill/Modify additional details and press "Submit"</span>
              <Typography
                as="h6"
                sx={{
                  fontSize: '14px',
              ...
              <TextField
                ...
                helperText={touched.screenerName && errors.screenerName}
                variant="outlined"
                fullWidth
                dense
                size="small"
