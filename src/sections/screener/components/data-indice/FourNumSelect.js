...
export default function FourNumSelect(props) {
  const {
    showInput,
    index,
    handleValue,
  ...
  const onDataChange = (value, field) => {
    ...
    if (field === 'indice') {
      ...
      const num4 = values?.numVal4 || values?.indice?.defaultNumVal4;
    ...
    <>
      ...
      <Box as="div" sx={{ display: 'flex', cursor: 'pointer' }}>
        <Stack direction="row" sx={{ alignItems: 'center', flexDirection: ' center' }}>
          ...
          <FilterTextfield
            isDisabled={isDisabled}
            value={values.numVal4}
            enclose={{ open: false, close: true }}
            showArrow
            handleChange={(val) => onDataChange(val, 'numVal4')}
            // valSx={{ color: !isDisabled ? 'secondary.main' : '#828282', fontWeight: 700 }}
            handleBlur={(val) => {
              handleBlur(val, index);
              markComplete();
