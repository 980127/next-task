...
const defaultIndiceVal = _.find(dataPointOptions, (point) => point?.name === 'Close');

export default function SmaSelect(props) {
  const {
    showInput,
    index,
    handleValue,
  ...
  const onDataChange = (value, field) => {
    ...
    if (field === 'indice') {
      const num = values?.numVal || '20';
      upVal.indice = { ...upVal?.indice };
      upVal.subIndice = [{ type: 'data', indice: defaultIndiceVal, candle: defaultCandleVal, isComplete: true }];
      upVal.numVal = num;
  ...
  return (
    <>
      ...
      <Box as="div" sx={{ display: 'flex', cursor: 'pointer' }}>
        <Stack
          direction="row"
          sx={{ alignItems: 'center', flexDirection: ' center', flexWrap: { xs: 'wrap', sm: 'wrap', md: 'nowrap' } }}
        >
