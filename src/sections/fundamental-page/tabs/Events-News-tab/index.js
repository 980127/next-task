...
export default function EventsNews() {
  ...
  useEffect(() => {
    if (stockNewsData && dateValue && dateValue[1]) {
      const filterDate = stockNewsData.filter(a => {
        return (moment(a.publishedDate).isAfter(moment(dateValue[0]).subtract(1, 'days')) && moment(a.publishedDate).isBefore(moment(dateValue[1]).add(1, 'days')));
      });
  ...
  const { tickerNews = [], tickerDividendAndSplit = {}, loadMoreNewsUrl } = useSelector(({ stockReducer }) => stockReducer);
    ...
    <Box>
      ...
      >
        <Stack direction="row" sx={{ textAlign: 'center', alignItems: 'center', }}>
          ...
          />
          <Typography sx={{ fontSize: '12px', lineHeight: '18px', fontWeight: 500, color: '#302F42', marginLeft: '10px' }}>
            Date Range
          </Typography>
        ...
        <KeyboardArrowDownIcon sx={{ fontSize: '14px' }} /> */}
        <DateRangeOutlinedIcon sx={{ pl: '10px', height: '20px' }} />
        <Typography sx={{ fontSize: '12px', lineHeight: '18px', fontWeight: 500, color: '#302F42' }}>
          All news
        </Typography>
        <KeyboardArrowDownIcon sx={{ fontSize: '14px' }} />
            ...
            <>
              <Stack direction="row">
                <Box
                  component="img"
                  src={image}
                  sx={{ maxWidth: { xs: '100px', lg: '200px' }, width: 'auto', objectFit: 'stretch', }}
                />
      ...
      <Stack
        direction="row"
        sx={{ textAlign: 'center', alignItems: 'center', justifyContent: 'flex-end', cursor: 'pointer', paddingRight: { xs: '14px' } }}
      >
        <Typography sx={{ fontSize: '12px', color: '#3886FA', lineHeight: '30px', fontWeight: 500 }} onClick={onLoadMoreClick}>
          Load more
        </Typography>
