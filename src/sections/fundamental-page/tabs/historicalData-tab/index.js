...
export default function HistoricalDataTab(props) {
  ...
  useEffect(() => {
    if (historicalChart && dateValue) {
      const filterDate = historicalChart.filter(a => {
        return (moment(a.date).isAfter(moment(dateValue[0]).subtract(1, 'days')) && moment(a.date).isBefore(moment(dateValue[1]).add(1, 'days')));
      });
      ...
      <Stack direction="row" spacing={1} sx={{ textAlign: 'center', alignItems: 'center', padding: '0px', justifyContent: 'space-between' }} >
        ...
        <Stack direction="row" sx={{ textAlign: 'center', alignItems: 'center', }}>
          <FormControl variant="standard" sx={{ m: 1, minWidth: 120 }}>
            ...
            >
              <MenuItem sx={{ fontSize: '12px', color: '#302F42', fontWeight: 400, fontFamily: 'Poppins', }} value="">
                <em>None</em>
              </MenuItem>
